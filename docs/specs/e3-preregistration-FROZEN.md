...
Status: **CONGELADO (FROZEN) em 2026-06-28**, ANTES de qualquer recrutamento/dado. Selo = hash do commit
git desta versão (imutável no histórico) + sha256 do conteúdo registrado na mensagem do commit. Alteração
posterior exige nova versão datada + justificativa (e re-hash). Tier primário = Tier 1 honesto. Origem:
council de metodologia + decisões do dono 2026-06-27/28 ([[launch-readiness-council-2026-06-27]]).

> Nota de implementação: a metodologia abaixo está congelada. O código do estimador (correção RTM/AR(1))
> ainda será implementado e DEVE conformar-se a este spec, validado em dado SIMULADO antes de rodar na
> coorte real (que está a ≥60d). Detalhe de implementação que não altere hipótese/limiar/janelas/dose não
> quebra o pré-registro; mudança nesses = amendamento datado + justificado.
...
## 3. Limiar de efeito praticamente relevante  ✅ FIXADO (dono, 2026-06-28)
**+50 pontos Glicko rapid** na inclinação pós vs pré em 90 dias (após correção de RTM), como limiar de
relevância prática. Racional: ~meia faixa de banda; abaixo disso o sinal se confunde com ruído de Glicko
para quem joga poucas partidas/mês. Abaixo do limiar → resultado é "não-relevante" (frase do §8).

## 4. Dose mínima (gate de elegibilidade)  ✅ FIXADO (dono, 2026-06-28)
**≥ 8 sessões de treino concluídas em ≥ 6 dias distintos** dentro da janela pós (90d). Abaixo da dose, o
usuário NÃO entra na análise primária (entra em análise de intenção-de-tratar separada). Racional: dose
mínima plausível para esperar transferência ao jogo real sem exigir uso intenso.
