# DeepSeek - Analise Dos Livros Convertidos (PT-BR Kindle/EPUB -> Texto)

Voce e o **DeepSeek** atuando como pesquisador pedagogico senior do projeto `lichess-tutor`. Data de
...
> Identifique-se: comece com `# Analise dos Convertidos - DEEPSEEK` e salve como
> `analise-convertidos-DEEPSEEK.md`. O mesmo prompt sera rodado por Gemini e Codex em paralelo e depois
...
- **DAMP** (`DAMP-O-Algoritmo...txt`): confirme no texto o que significam as 4 letras. Uma analise
  anterior (Gemini) afirmou "Descobrir/Ameacar/Modificar/Prevenir" — verificacao no indice do livro
  sugere **D=Defesa, A=Alinhamento, M=Mobilidade, P=Promocao**, um checklist de **deteccao tatica**
  (onde existe tatica), nao um ritual de seguranca. **Confirme ou corrija com base no texto.**
- **Como montar uma programacao de treinamento de xadrez** (Lapertosa): metodo de divisao de tempo de
  estudo (time budgeting) em PT-BR.
...
- Separe sempre **(E)** evidencia, **(I)** inferencia, **(P)** decisao de produto.
...
1. **Nota dos convertidos** (0-10) como aporte ao metodo. Justifique.
